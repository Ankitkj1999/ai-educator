...
app.post('/api/prompt', async (req, res) => {
    ...
    if (!promptString) {
        console.error('Error in /api/prompt: Received empty prompt string.');
    ...
    try { // <-- Optional: Wrap model initialization in try...catch too
        const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash", safetySettings });
        const prompt = promptString;

        console.log(`[${new Date().toISOString()}] Attempting to generate content for prompt: "${promptString.substring(0, 50)}..."`); // Log the attempt

        await model.generateContent(prompt).then(result => {
            const response = result.response;

            // --- Check if the response or text is missing ---
            if (!response) {
                 console.error('Error in /api/prompt: Gemini API returned undefined response object.');
                 return res.status(500).json({ success: false, message: 'Internal server error: Invalid API response.' });
            }
            const generatedText = response.text();
             if (typeof generatedText !== 'string') { // Check if text() method returned something usable
                 console.error('Error in /api/prompt: Gemini API response.text() did not return a string. Response:', response);
                 ...
                 if (response.promptFeedback?.blockReason) {
                    ...
                    return res.status(400).json({ success: false, message: `Request blocked: ${response.promptFeedback.blockReason}` });
                 }
                 return res.status(500).json({ success: false, message: 'Internal server error: Could not extract text from API response.' });
            }
            // ----------------------------------------------

            console.log(`[${new Date().toISOString()}] Successfully generated content for prompt.`); // Log success
            res.status(200).json({ generatedText });

        }).catch(error => {
            // <<<--- LOG THE ERROR HERE ---<<<
            console.error(`[${new Date().toISOString()}] Error during model.generateContent in /api/prompt:`, error);
            // You might want to log specific parts of the error if available
            // console.error('Error details:', error.message, error.stack);
            // >>>-------------------------->>>

            res.status(500).json({ success: false, message: 'Internal server error generating content.' }); // Slightly more specific message
        });

    } catch (initError) { // <-- Catch errors during model initialization
        console.error(`[${new Date().toISOString()}] Error initializing Generative Model in /api/prompt:`, initError);
        res.status(500).json({ success: false, message: 'Internal server error initializing AI model.' });
    }
});
